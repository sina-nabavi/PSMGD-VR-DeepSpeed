...
from LibMTL import Trainer_DS
from LibMTL.model import resnet_dilated
from LibMTL.utils import set_random_seed, set_device
from LibMTL.config import LibMTL_args, prepare_args
from LibMTL.config_DS import parse_ds_args
from LibMTL.config_DS import get_ds_config
import deepspeed
...
def main(params):
    ...
    # Initialize DeepSpeed distributed backend + add DeepSpeed config
    deepspeed.init_distributed()
    _local_rank = int(os.environ.get("LOCAL_RANK"))
    get_accelerator().set_device(_local_rank)
    ds_config = get_ds_config(params)
    ...
    if params.mode == 'train':
        NYUmodel.train(nyuv2_train_set, nyuv2_test_loader, params.epochs)
...
if __name__ == "__main__":
    parser = parse_args(LibMTL_args)
    params = parse_ds_args(parser)
    # set device
    #single GPU use only 
