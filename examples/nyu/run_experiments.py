...
#combinations = list(product(n_values, freqs, alphas, learning_rates, seeds))
combinations = list(product(seeds,alphas, list(itertools.product(n_values,q_values))))
...
while combinations or running_processes:
    ...
    for gpu in gpus:
        while gpu_occupancy[gpu] < max_processes_per_gpu and combinations:
            seed, alpha, (n_val, q_val) = combinations.pop(0)
            freq = q_val
            # Set the environment variable for this process
            env = os.environ.copy()
            env['CUDA_VISIBLE_DEVICES'] = gpu
            # Output filename
            output_file = f'./PSMGDVR_lower_lr/PSMGDVR_n_{n_val}_q_{q_val}_freq_{freq}_alpha_{alpha}_seed_{seed}.txt'
            ...
            print(f"Started: PSMGDVR n {n_val} freq {freq} alpha {alpha} lr {learning_rate} seed {seed}, on GPU {gpu}")
