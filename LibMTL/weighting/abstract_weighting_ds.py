...
class AbsWeighting(nn.Module):
    ...
    def _compute_grad(self, losses, mode, rep_grad=False, backward=None):
        '''
        ...
        if not rep_grad:
            ...
            for tn in range(self.task_num):
                if mode == 'backward':
                    ...
                    backward(losses[tn],retain_graph=True) if (tn+1) != self.task_num else backward(losses[tn])
            ...
            for tn, task in enumerate(self.task_name):
                if mode == 'backward':
                    #losses[tn].backward(retain_graph=True) if (tn+1)!=self.task_num else losses[tn].backward()
                    backward(losses[tn],retain_gaph=True) if (tn+1)!=self.task_num else backward(losses[tn])
