...
class PSMGD(AbsWeighting):
    r"""Periodic Stochastic Multi-Gradient Descent with variance reduction (PSMGD-VR).
    
    This method does not use a specialized stochastic optimzation such as SDMGrad. 

    """

    def __init__(self):
        super(PSMGD, self).__init__()
    ...
    def init_param(self):
        self.step=0
        self.ut_pre = None
        self.lambda_t_pre = None
        
    def backward(self, losses,prev_train_losses, **kwargs):
        self.alpha = torch.tensor(kwargs['alpha']).to(self.device)
        ...
        if self.step % self.freq == 0:
            ...
            grads = self._gradient_normalizers(grads, loss_data, ntype=kwargs['psmgd_gn']) # l2, loss, loss+, none
            lambda_star = self._find_min_norm_element(grads).to(self.device)
            if self.step > 0:
                lambda_star = (1 - self.alpha)* lambda_star
                lambda_star += self.alpha* self.lambda_t_pre
                
            if self.rep_grad:
                self._backward_new_grads(lambda_star, per_grads=per_grads)
                self.ut_pre = per_grads
            else:
                self._backward_new_grads(lambda_star, grads=grads)
                self.ut_pre = grads

            self.lambda_t_pre = lambda_star
        else:
            prev_grads = self._get_grads(prev_train_losses, mode='backward')
            if self.rep_grad:
                prev_per_grads, prev_grads = prev_grads[0], prev_grads[1]
            prev_loss_data = torch.tensor([loss.item() for loss in prev_train_losses]).to(self.device)
            prev_grads = self._gradient_normalizers(prev_grads, prev_loss_data, ntype=kwargs['psmgd_gn']) # l2, loss, loss+, none
            psmgd_gn = kwargs['psmgd_gn']
            grads = self._get_grads(losses, mode='backward')
            if self.rep_grad:
                per_grads, grads = grads[0], grads[1]
            loss_data = torch.tensor([loss.item() for loss in losses]).to(self.device)
            grads = self._gradient_normalizers(grads, loss_data, ntype=psmgd_gn) # l2, loss, loss+, none
            if self.rep_grad:
                # previous model but the new data samples
                ut = self.ut_pre + per_grads - prev_per_grads
                self._backward_new_grads(self.lambda_t_pre, per_grads=ut) 
            else:
                # previous model but the new data samples
                ut = self.ut_pre + grads - prev_grads
                self._backward_new_grads(self.lambda_t_pre, grads=ut) 
            self.ut_pre = ut
        self.step+=1